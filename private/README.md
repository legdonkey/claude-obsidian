...
本文档记录 `legdonkey/claude-obsidian` fork 的远端、分支、升级和私有版本规则。目标是让私有化定制可以长期维护，同时持续跟踪 upstream 的稳定版本。

> 关系速记：`origin` = 我自己的 fork（`legdonkey/claude-obsidian`）；`upstream` = 原作者仓库（`AgriciDaniel/claude-obsidian`），只读、只拉不推。
...
```bash
git remote add upstream git@github.com:AgriciDaniel/claude-obsidian.git
...
- `origin`: `git@github.com:legdonkey/claude-obsidian.git`
- `upstream` fetch: `git@github.com:AgriciDaniel/claude-obsidian.git`
...
格式 `v<官方版本>-private.<N>`。规则：

- `<官方版本>` 必须对应真实 upstream tag（如 `1.9.2`）
...
- 官方版本号体现在：`.claude-plugin/plugin.json`（字段 `version`），以及 upstream 的 git tag。
